...
INI
  ...
  like(
    $err,
    qr/^substr: .*isn't numeric in substr/,
    'offset warning converted to substr error',
  );

  unlike(
    $err,
    qr/\sat\s+\S+\s+line\s+\d+/,
    'offset error has no file/line tail',
  );
...
INI
  ...
  like(
    $err,
    qr/^substr: .*isn't numeric in substr/,
    'length warning converted to substr error',
  );

  unlike(
    $err,
    qr/\sat\s+\S+\s+line\s+\d+/,
    'length error has no file/line tail',
  );
};
